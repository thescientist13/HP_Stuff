...
import { parseGedcom, readGedcom, SelectionGedcom } from 'read-gedcom';
...
export class gedcomDataControler implements ReactiveController {
        ...
        this.loadGedcomUrl(url)
          .then((b) => {
            if(typeof b !== 'undefined') {
              let root = this.readGedcomFromBuffer(b);
              if(root) {
                console.log(`root successfully populated in loadGedcomUrl`)
                const other = this.initializeAllFields(root);
              } else {
  ...
  public readGedcomFromBuffer = action(gedData, 'readGedcomFromBuffer',  (store, buffer: ArrayBuffer) => {
      ...
      return parseGedcom(buffer);
      
    });
  
  protected initializeAllFields = action(gedData, 'initializeAllFields', (store, root: TreeNodeRoot) => {
  ...
  protected computeDependantFields = action(gedData, 'computeDependantFields', (store, root, rootIndividual) => {
