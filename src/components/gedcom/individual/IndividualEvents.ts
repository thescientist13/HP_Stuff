...
import { z} from "zod";
import {SelectionEvent, SelectionIndividualRecord} from 'read-gedcom';
...
export class IndividualEvents extends TailwindMixin(LitElement,style) {
    ...
    @state()
    private individual: SelectionIndividualRecord | null;

    @state()
    private i2: SelectionIndividualRecord | null;
    ...
    private renderEventPlace(event: SelectionEvent, t: TemplateResult<1>) {
        if(this.showPlace) {
    ...
    render() {
        if (this.gcDataController && this.gcDataController.gedcomStoreController && this.gcDataController.gedcomStoreController.value) {
            if(this.showMarriage){
                if(this.gedId && this.gedId2) {
                    this.individual = this.gcDataController.getIndividualRecord(this.gedId);
                    this.i2 = this.gcDataController.getIndividualRecord(this.gedId2);
                    if(this.individual && this.i2) {
                        ...
                        const family = this.individual.getFamilyAsSpouse().filterSelect(f => {
                            ...
                            const w = f.getWife();
                            if((h.getIndividualRecord().pointer()[0] === this.gedId) || (w.getIndividualRecord().pointer()[0] === this.gedId)) {
                                if((h.getIndividualRecord().pointer()[0] === this.gedId2) || (w.getIndividualRecord().pointer()[0] === this.gedId2)) {
                                    return true;
                                }
                            }
                            return false;
